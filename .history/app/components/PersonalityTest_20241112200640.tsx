...
const PersonalityTest = () => {
  ...
  // Initialize scores at the start of the test
  useEffect(() => {
    const initialScores = questions.reduce((acc, question) => {
      question.options.forEach(option => {
        ...
        if (!acc[option.category][option.subcategory]) {
          acc[option.category][option.subcategory] = 0;
        }
      });
      return acc;
    }, {} as Record<string, Record<string, number>>);

    localStorage.setItem("ikigaiScores", JSON.stringify(initialScores));
  }, []);
  ...
  const handleSubmit = () => {
    if (selectedOption) {
      ...
      console.log("Updated Scores: ", updatedScores); // Log the updated scores
  ...
  const questions: { question: string; options: { label: string; category: string; subcategory: string }[] }[] = [
    ...
    {
      question: "Your family would describe you as:",
      options: [

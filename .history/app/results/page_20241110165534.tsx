...
const generateDynamicSummary = (dominantCategory: string, secondaryCategory: string, subcategories: string[]) => {
  ...
  const subcategoryPhrasesList = subcategories.map(subcategory => subcategoryPhrases[subcategory] || []).flat().slice(0, 2);
...
const renderActiveShape = (props: any) => {
  ...
  return (
    <g>
      <text x={cx} y={cy} dy={8} textAnchor="middle" fill={fill} fontSize={fontSize}>{payload.name}</text>
...
const ResultsPage = () => {
  ...
  useEffect(() => {
    const storedScores = JSON.parse(localStorage.getItem("ikigaiScores") || "null");
    ...
    for (const category in categoryTotals) {
      categoryTotals[category as Category] = Object.values(scores[category as Category]).reduce((acc, val) => acc + val, 0);
    }
...
function formatSubcategoryLabel(subcategory: string): any {

...
const ResultsPage = () => {
  ...
  useEffect(() => {
    const storedScores = localStorage.getItem("ikigaiScores");
    if (storedScores) {
      setScores(JSON.parse(storedScores));
    } else {
      const defaultScores = initializeScores(); // Assuming initializeScores is defined elsewhere
      setScores(defaultScores);
      localStorage.setItem("ikigaiScores", JSON.stringify(defaultScores));
    }

    const categoryTotals: Record<Category, number> = {
      passion: 0,
    // Initialize default scores if not present
    if (!localStorage.getItem("ikigaiScores")) {
      const defaultScores = {
        passion: { subcategory1: 0, subcategory2: 0 },
        vocation: { subcategory1: 0, subcategory2: 0 },
        mission: { subcategory1: 0, subcategory2: 0 },
        profession: { subcategory1: 0, subcategory2: 0 },
      };
      localStorage.setItem("ikigaiScores", JSON.stringify(defaultScores));
    }
    ...
    // Ensure storedScores has valid structure
    if (!storedScores || Object.keys(storedScores).length === 0) {
      // Handle the case where storedScores is empty or not as expected
      console.warn("storedScores is empty or undefined. Initializing default scores.");
      return;
    }
